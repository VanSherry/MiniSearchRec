...
包含完整的 **LightGBM 训练闭环**：在线行为上报 → 事件落盘 → **后台自动**离线样本导出 → LambdaRank 训练 → **双 Buffer 热更新（服务不停机切换模型）**。**BackgroundScheduler 后台线程自动执行训练+索引重建，无需 cron 或手动操作。**
...
| 向量召回 | Faiss HNSW（无 Faiss 时自动降级暴力余弦搜索），**EmbeddingProvider 配置驱动（默认 64 维伪 embedding，可一键切换真实模型）**，向量召回开箱即用 | ✅ 运行（可选 Faiss） |
...
| Query 理解 | 分词/归一化/停用词/同义词扩展/类别推断/**EmbeddingProvider 统一生成 query embedding（配置切换 pseudo/http）** | ✅ 运行 |
...
| **后台自动训练** | BackgroundScheduler 后台线程：事件驱动+定时兜底 → fork 子进程 dump+train → 双 Buffer 热更新，**全自动无需 cron** | ✅ 运行 |
| **后台自动索引重建** | 定时全量重建倒排+向量索引 → `shared_ptr` 原子切换 → 旧索引自动释放，**不停服** | ✅ 运行 |
| **优雅退出** | SIGTERM/SIGINT 信号处理 → Stop HTTP → Stop Scheduler → 等待当前任务完成 | ✅ 运行 |
| 集成测试 | `test_all`：63 项测试覆盖所有修复点，不依赖 GTest | ✅ 63/63 通过 |
...
离线训练（BackgroundScheduler 自动执行，无需 cron）
  ↓ 触发条件：事件数 ≥ min_events 或超时 interval_hours
  ↓ dump_train_data：events.db → data/train.txt（fork 子进程，隔离执行）
  ↓ train_rank_model.py：LambdaRank 训练（fork 子进程，崩溃不影响主服务）
  ↓ 双 Buffer 热更新：LGBMScorerProcessor 原子切换新 Booster，推理不中断

索引自动重建（BackgroundScheduler 定时执行）
  ↓ 从 DocStore 全量读取文档
  ↓ 独立内存中构建新 InvertedIndex + VectorIndex
  ↓ shared_ptr 原子切换，旧索引引用计数归零后释放
```
...
# Embedding 向量服务（配置驱动，一键切换）
embedding:
  provider: "pseudo"       # pseudo（内置伪向量） / http（远程模型服务）
...
recall_stages:
  ...
  - name: "VectorRecallProcessor"
    ...
    params:
      ...
      embedding_dim: 64            # 与 global.yaml 的 embedding.dim 保持一致
```
...
| `test_all` 63 项集成测试 | 质量保障 | Integration Testing |
...
| 向量召回 `query_embedding` 永远为空，100% 跳过 | **EmbeddingProvider 抽象层**：`global.yaml` 配置 `embedding.provider`（pseudo/http）+ `embedding.dim`，QueryParser/IndexBuilder 统一调用，一键切换真实模型无需改代码 |
...
| 无集成测试 | `tests/test_all.cpp`：63 项测试，不依赖 GTest，全部通过 |
