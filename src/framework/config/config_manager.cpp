...
bool ConfigManager::LoadGlobalConfig(const std::string& path) {
    try {
        ...
        if (config["embedding"]) {
            auto emb = config["embedding"];
            global_config_.embedding.provider =
                emb["provider"].as<std::string>("onnx");
            global_config_.embedding.dim =
                emb["dim"].as<int>(768);
            global_config_.embedding.model_path =
                emb["model_path"].as<std::string>("");
            global_config_.embedding.tokenizer_path =
                emb["tokenizer_path"].as<std::string>("");
            global_config_.embedding.max_seq_len =
